...
推荐把指纹样本放入 `case/fixtures/fingerprint.fixture.json`，或合并进业务 fixture 的 `fingerprint` 字段。
...
1. 用 RuyiTrace NDJSON 或 Hook 找到目标 JS 是否访问指纹 API。
2. 如果访问的是 Canvas / WebGL / WebGPU / Audio / 字体 / DOM 几何，先生成采样 Hook：

   ```bash
   node scripts/generate_fingerprint_hook.js --types canvas,webgl,dom-geometry --out case/hooks/fingerprint-hook.js
   ```

3. 在用户确认的取证工具中运行采样 Hook，触发最少量业务动作。
4. 把采样结果保存为 `case/fixtures/fingerprint.fixture.json`。
5. 运行覆盖检查：
...
6. 把 `assets/env-modules/fingerprint-env.js` 复制到最终项目 `result/src/env/`，按 fixture 接入。
7. 对目标 JS 运行 fixtures，若缺样本，补采样而不是改用自动化。
8. 交付前运行：
