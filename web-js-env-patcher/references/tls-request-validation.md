...
当新 case 需要最终发送真实请求、交付 `final.js` / `final.py`，或用户提到 CycleTLS、impers、curl_cffi、cffi_curl、cyCronet 时读取本文件。TLS 指纹兼容客户端的选择应从前置阶段开始确认，不要等普通 `fetch` / `requests` 失败后才临时切换。
...
- 最终真实请求必须写入一体化 `final.js` 或 `final.py`，由 Node.js / Python 请求客户端直接发起；不得生成加密参数后再使用 ruyiPage、Playwright、Puppeteer、Selenium、CloakBrowser 或其他浏览器自动化验证。
...
async function sendRequestWithCycleTLS({ url, method = 'GET', headers = {}, body, ja3, userAgent }) {
...
module.exports = { sendRequestWithCycleTLS };
```
...
def send_request_with_curl_cffi(url, method="GET", headers=None, data=None, json_data=None, impersonate="chrome"):
...
5. 使用已确认的 CycleTLS / impers / curl_cffi / cffi_curl / cyCronet 仅发起少量验证请求；若用户选择“不发真实请求”，则只输出本地 sign / 参数。
6. 对比：
   - HTTP 状态码。
   - 响应 JSON 中关键字段。
   - 服务端是否接受新生成的加密参数。
...
- 是否执行真实请求：是 / 否
- 用户授权：是 / 否
- 前置阶段已选客户端：Node.js CycleTLS / Node.js impers / Node.js curl-cffi / Python curl_cffi / Python cffi_curl / Python cyCronet / 不发真实请求
